      ...
      <div className="pt-2 lg:pt-4 text-center">
        ...
        <p className="my-1 text-center text-xs md:text-base">
          {data?.CV?.data?.attributes.url && (
            <a
              target="_blank"
              rel="noreferrer"
              href={data.CV.data.attributes.url}
            >
              Download CV
            </a>
          )}
        ...
        <div className="text-xs md:text-base">
          <div className="text-xs md:text-base no-underline text-blue-800">
            {data.Interview ? (
              <a href={data.Interview} rel="noreferrer" target="blank">
...
const CandidateDashboard = () => {
  ...
  const FilteredCandidates = (candidates) => {
    return candidates
      ...
      .filter((candidate) => {
        if (
          candidate.attributes.technologiesNew.data.length === 0 &&
          !isAllTag &&
          tags.every((tag) =>
            candidate.attributes.Technologies.technologies.some(
              (tech) =>
